    ...
    def __init__(self, sequence_length, target_sequence_length,targets_num_max, num_classes, word_embedding, l2_reg_lambda=0.0,
        ...
        with tf.variable_scope("Attention-targets_all2sentence") as scope:
            ...
            for i in range(targets_num_max):
                ...
                att_s_i = bilinear_attention_layer(LSTM_Hiddens_sen_p_i, poor_targets_all[i], self.sen_len, 2 * num_hidden ,l2_reg_lambda,
                                             random_base = rand_base, layer_id = 'sen')
                self.outputs_ts[i] = tf.squeeze(tf.matmul(att_s_i, self.LSTM_Hiddens_sen),axis=1)   #13*(?,600)
                scope.reuse_variables()
...
class LSTM_CAtt_GCN2_positionSSS_2(object):
    '''
    唯一的不同是attention是针对与和place运算过，还是原来的sentence hidden 值。
    '''
    def __init__(self, sequence_length, target_sequence_length,targets_num_max, num_classes, word_embedding, l2_reg_lambda=0.0,
                 num_hidden=100):
        #tf.set_random_seed(-1)
        ...
        with tf.variable_scope("Attention-targets_all2sentence") as scope:
            ...
            for i in range(targets_num_max):
                ...
                att_s_i = bilinear_attention_layer(LSTM_Hiddens_sen_i, self.outputs_ss[i], self.sen_len, 2 * num_hidden ,l2_reg_lambda,
                                             random_base = rand_base, layer_id = 'sen')
                self.outputs_ts[i] = tf.squeeze(tf.matmul(att_s_i, LSTM_Hiddens_sen_i), axis=1)
                scope.reuse_variables()
