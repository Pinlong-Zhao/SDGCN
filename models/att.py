# -*- coding: utf-8 -*-
#only aspect2sentence attention
...
class Att(object):
    def __init__(self, sequence_length, target_sequence_length,targets_num_max, num_classes, word_embedding, l2_reg_lambda=0.0,
        ...
        with tf.name_scope("Attention-target2sentence"):
            ...
            self.att_s = bilinear_attention_layer(LSTM_Hiddens_sen_p, pool_target, self.sen_len,2 * num_hidden ,l2_reg_lambda,
                                             random_base = rand_base, layer_id = 'sen')   #(?,1,78)

            self.outputs_t = tf.squeeze(tf.matmul(self.att_s, self.LSTM_Hiddens_sen), axis=1)     #(?,1,78)* (?,78,600)----->(?,600)
